...
func open(ctx context.Context, system opener, timeout time.Duration) (Vault, Status) {
	ctx, cancel := context.WithTimeout(ctx, timeout)
	defer cancel()

	answers := make(chan answer, 1)
	go func() {
		vault, err := system(ctx)
	...
	case got := <-answers:
		...
		return got.vault, Status{Backend: systemBackend}
	case <-ctx.Done():
...
func fallback(reason error) (Vault, Status) {
	return session{secret.NewMemory()}, Status{
		...
		Warning: fmt.Sprintf(
			...
			systemBackend, reason, systemAdvice),
	}
}

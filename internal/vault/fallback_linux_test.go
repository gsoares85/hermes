...
// written is the reference the fallback is asked to store under. Which one it
// is does not matter; that a write happened at all does, because the assertion
// is about what it left on the disk.
var written = secret.Ref{Service: secret.Service, Account: "fallback-write"}
...
func TestWithoutASecretServiceTheVaultFallsBackToMemory(t *testing.T) {
	...
	if err := vault.Set(t.Context(), written, "hunter2"); err != nil {
